...
export class Input{
  constructor(){this.keys=new Set();this.touch={left:false,right:false,gas:false,brake:false,drift:false,nitro:false};this._bind()}
  _bind(){
  ...
  sample(){
    ...
    if(p){
      const dz=v=>Math.abs(v)<.12?0:v;steer=clamp(dz(p.axes[0]||0),-1,1);
      const gas=Math.max(p.buttons[7]?.value||0,p.buttons[0]?.pressed?.7:0),brake=Math.max(p.buttons[6]?.value||0,p.buttons[1]?.pressed?.7:0);
    ...
    return{throttle:clamp(throttle,-1,1),steer:clamp(steer,-1,1),handbrake,nitro}
  }
}
