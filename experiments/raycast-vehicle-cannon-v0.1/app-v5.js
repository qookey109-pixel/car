...
const lensClear=new THREE.MeshPhysicalMaterial({
  color:0xcff7ff,roughness:.04,metalness:.02,transparent:true,opacity:.58,transmission:.18,
...
function lens(x,y,z,scale,mat,rotY=0){
  const m=new THREE.Mesh(new THREE.SphereGeometry(1,12,7),mat);
...
function arch(side,z,r=.49){
  // recessed black wheel cavity + body-color outer lip + carbon inner lip
  const cavity=new THREE.Mesh(new THREE.CircleGeometry(r*.97,24),wellMat);
  cavity.position.set(side*1.102,.16,z);cavity.rotation.y=side>0?-Math.PI/2:Math.PI/2;car.add(cavity);
  const outer=new THREE.Mesh(new THREE.TorusGeometry(r,.052,7,28,Math.PI),paint);
  outer.position.set(side*1.112,.17,z);outer.rotation.set(0,Math.PI/2,side>0?Math.PI:0);outer.castShadow=true;car.add(outer);
  const inner=new THREE.Mesh(new THREE.TorusGeometry(r-.055,.027,5,24,Math.PI),carbon);
  inner.position.set(side*1.119,.17,z);inner.rotation.set(0,Math.PI/2,side>0?Math.PI:0);car.add(inner);
}

// Deeper wheel arches and wider fender shoulders.
for(const side of [-1,1]){
  ...
  // pronounced lower side skirt with a small aero blade
  ...
  // shoulder crease: helps the side surface read in motion
  car.add(box(.018,.022,2.58,seam,side*1.105,.39,.18,0,0,0));
}
...
// hood power bulge and sharper leading edge
...
// Real volumetric headlamp lenses with separate internal projector cores.
for(const side of [-1,1]){
  const x=side*.62;
  car.add(lens(x,.225,-2.405,[.31,.105,.075],lensClear,side*.08));
  const core=new THREE.Mesh(new THREE.SphereGeometry(.08,10,7),new THREE.MeshStandardMaterial({
...
// Rear bumper gets deeper surfaces, garnish and dimensional tail lenses.
...
// license recess and tow-hook detail
car.add(box(.58,.18,.035,wellMat,0,.02,2.438));
const tow=new THREE.Mesh(new THREE.TorusGeometry(.075,.018,6,14),new THREE.MeshStandardMaterial({color:0xe83936,roughness:.38,metalness:.5}));
tow.position.set(.58,-.10,2.45);tow.rotation.x=Math.PI/2;car.add(tow);

// Underbody: visible only from bumps / pitch, but adds believable mass.
...
// Small roof / window sealing lines create real panel separation.
car.add(box(1.22,.024,.024,seam,0,.90,-.28));
car.add(box(1.20,.024,.024,seam,0,.89,.55));

// Wheel V5: outer rim ring, visible wheel nuts and low-poly tread blocks.
for(const wheel of wheels){
  ...
  for(let n=0;n<5;n++){
    ...
    const nut=new THREE.Mesh(new THREE.CylinderGeometry(.012,.012,.342,6),metal);
    nut.rotation.z=Math.PI/2;nut.position.set(0,Math.cos(a)*.055,Math.sin(a)*.055);wheel.add(nut);
  }
  for(let t=0;t<14;t++){
    const a=t*Math.PI*2/14;
    const block=box(.305,.040,.105,wellMat,0,Math.cos(a)*.355,Math.sin(a)*.355,a,0,0);
    wheel.add(block);
  }
}

// Version wrapper: physics snapshot stays V4-identical; only visual identity changes.
...
lab.visualDetails={
  ...
  tires:'14-block-tread',underbody:true,bumperDepth:true
};
